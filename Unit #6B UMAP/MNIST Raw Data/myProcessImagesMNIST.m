function X = myProcessImagesMNIST(filename)
...
% RTB modified original code to return format used by JP for the UMAP
% exercise. 22 September 2022
...
X = permute(X,[3 2 1]);
X = reshape(X, numImages, numCols*numRows);
% Images are between 0 and 255, so we can save memory:
X = uint8(X);

% Original code from processImagesMNIST.m
% X = permute(X,[2 1 3]);
% X = X./255;
