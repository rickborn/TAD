...
% one-tailed p-value:
% How likely is pure bias to have produced an effect as large or larger
% than the one we actually obtained?
...
%
% ANSWER: This is equivalent to asking for what # of questions-per-seminar
% are 95% of our simlulated values < 25. We could do this with a 'while'
% loop:
...
while propLT25 < 0.95
    ...
    for k = 1:nSims
        % Simulate data: each row is a question, each column a seminar. Assume a
        % value of '1' means a woman asked the question; '0' means a man asked it.
        allData = round(rand(nQtest,nSeminars));
        ...
        % vector, a=[1,2,3,4], 'sum(a)' will return 10, whereas 'sum(a,1)' will
        % return [1 2 3 4]. This is a teachable moment.
        simMFscores = round(((sum(simManFirst,1) ./ nQtest) - 0.5) * 100);
        simWFscores = round(((sum(simWomanFirst,1) ./ nQtest) - 0.5) * 100);
        
        allEffectSizes(k) = mean(simWFscores) - mean(simMFscores);
    end
    
    % Now we just ask how much of our simulated distribution is below the
    % actual value we obtained (i.e. 25)
    propLT25 = sum(allEffectSizes < realEffectSize) / nSims;
    
end

disp(['Min. # of questions for 95% confidence: ' num2str(nQtest)]);  % Answer is 5
